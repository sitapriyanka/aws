# aws# Notes for AWS Certified Solutions Architect Associate
...
sqs purges all unused quese after 30 days
Will queues that are used as DLQs be deleted?

No. A queue used as a DLQ will not be deleted as long as any of its sources
still exist.

What if there are messages left in the queue?

The maximum message lifetime allowed by SQS is 14 days. If the queue has not been accessed for 30 days all messages in it will already have expired.

How can I tell if a queue still exists?

We recommend calling GetQueueUrl to determine is a queue exists. If the queue does not exist you can recreate it by calling CreateQueue. We recommend against calling CreateQueue every time as GetQueueUrl is significantly faster than CreateQueue. We also recommend against using ListQueues to determine if a queue exists as ListQueues is eventually consistent and so may include queues that were recently deleted.
